...
namespace StaffSync
{
    public partial class frmIndPayrollMaster : Form
    {
        ...
        frmPayrollBatchProcess frmPayrollBatchProcess = null;
        bool AutomaticallyCloseMe = false;
        public Dictionary<string, decimal> UpdatedSalaryValues = new Dictionary<string, decimal>();

        public frmIndPayrollMaster()
        {
            InitializeComponent();
        }

        //public frmIndPayrollMaster(UserRolesAndResponsibilitiesInfo objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo)
        //{
        //    InitializeComponent();
        //    objTempCurrentlyLoggedInUserInfo = objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo;
        //}

        //public frmIndPayrollMaster(UserRolesAndResponsibilitiesInfo objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo, ClientFinYearInfo objSelectedClientFinYearInfo)
        //{
        //    InitializeComponent();
        //    objTempCurrentlyLoggedInUserInfo = objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo;
        ...
        public frmIndPayrollMaster(string SearchOptionSelectedForm, UserRolesAndResponsibilitiesInfo objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo, ClientFinYearInfo objSelectedClientFinYearInfo, frmPayrollBatchProcess frmPayrollBtchProcess, int txtEmpID, string selectedMonth, DateTime dtSalaryDate, bool AutomaticallyCloseMeStatus)
        {
            InitializeComponent();
            objTempCurrentlyLoggedInUserInfo = objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo;
            objTempClientFinYearInfo = objSelectedClientFinYearInfo;
            ModelStaffSync.CurrentUser.ClientID = objTempClientFinYearInfo.ClientID;
            this.frmPayrollBatchProcess = frmPayrollBtchProcess;
            lblReportingManagerID.Text = txtEmpID.ToString();
            txtSalaryDate.Text = dtSalaryDate.ToString("dd-MM-yyyy");
            AutomaticallyCloseMe = AutomaticallyCloseMeStatus;

            lblActionMode.Text = "add";

            //SelectedEmployeeID1();
            SelectedEmployeeID(SearchOptionSelectedForm, txtEmpID, selectedMonth);
        }
        ...
        public void SelectedEmployeeID(string SearchOptionSelectedForm, int selectedEmployeeID, string cmbSalaryMonthSelectedText)
        {
            tabControl1.SelectedIndex = 0;
            if (SearchOptionSelectedForm == "listPayrollUsersList")
            {
                ...
                //cmbSalaryMonth.SelectedItem = cmbSalaryMonthSelectedText;
                //cmbSalaryMonth.Text = cmbSalaryMonthSelectedText;
                cmbSalaryMonth.SelectedIndex = cmbSalaryMonth.FindString(cmbSalaryMonthSelectedText);
                lblSalaryMonthSelected.Text = cmbSalaryMonthSelectedText;
                ...
                if (objAdvanceOutstandingList.Count > 0)
                {
                    ...
                    dtgAdvanceDetails.Columns["Select"].ReadOnly = true;
                    ...
                    dtgAdvanceDetails.Columns["RePaymentBalance"].ReadOnly = true;
                        ...
                        if (objAdvanceTypeConfigModel.RecoveryRequired)
                        {
                            if (objAdvanceTypeConfigModel.AutoDeductFromSalary)
                            {
                                if (objAdvanceTypeConfigModel.IncludeInSalary)
                                {
                                    ...
                                    {
                                        row.Cells["Select"].Value = false; //true
                                    }
                                    else
                                    {
                                        row.Cells["Select"].Value = false; //true
                            ...
                            else if (objAdvanceTypeConfigModel.AutoDeductFromSalary == false)
                            {
                                if (objAdvanceTypeConfigModel.IncludeInSalary)
                                {
                                    ...
                                    {
                                        row.Cells["Select"].Value = false; //false
                                    }
                                    row.Cells["Select"].ReadOnly = true;
                                    row.Cells["RePaymentBalance"].ReadOnly = true;
                                }
                ...
                DateTime parsedDate = DateTime.Parse("01-" + lblSalaryMonthSelected.Text.ToString().Replace(" ", ""), CultureInfo.InvariantCulture); //" DateTime.ParseExact(cmbSalaryMonth.SelectedItem.ToString(), "MMM - yyyy", CultureInfo.InvariantCulture);
                            ...
                            else
                            {
                                if (selectedClientStatutory.EnableClientStatutory)
                                {
                                    if (selectedClientStatutory.EnablePF == true)
                                    {
                                        ...
                                        lblPFCalcAmount.Text = total.ToString("#,#0.00");
                        ...
                        if (chkAutoCalculate.Checked == true)
                        {
                            ...
                            if (selectedClientStatutory.EnableClientStatutory)
                            {
                                if (selectedClientStatutory.EnablePT == true)
                                {
                                    ...
                                    int MonthNumber = months.ToList().FindIndex(s => s == lblSalaryMonthSelected.Text.ToString().Substring(0, 3).ToUpper()) + 1;
        ...
        private void cmbSalaryMonth_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            //{
            //    getMonthlyWorkingDays(Convert.ToInt16(lblReportingManagerID.Text.ToString()), parsedDate, parsedDate.AddDays(daysInMonth).Date);
            //    SelectedEmployeeID("listPayrollUsersList", Convert.ToInt16(lblReportingManagerID.Text.ToString()), cmbSalaryMonth.SelectedText.ToString());
            //}
        }
        ...
        private void frmIndPayrollMaster_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frmIndPayrollMaster_Activated(object sender, EventArgs e)
        {
            ...
            cmbSalaryMonth.SelectedIndex = cmbSalaryMonth.FindString(lblSalaryMonthSelected.Text);

            if (AutomaticallyCloseMe)
            {
                Thread.Sleep(200);

                UpdatedSalaryValues.Clear();

                foreach (DataGridViewRow row in dtgSalaryDetails.Rows)
                {
                    if (row.IsNewRow) continue;

                    string headerType = Convert.ToString(row.Cells["HeaderType"].Value);
                    string headerTitle = Convert.ToString(row.Cells["HeaderTitle"].Value);

                    decimal value = 0;

                    if (headerType.ToString().ToLower() == "allowences")
                        decimal.TryParse(row.Cells["AllowanceAmount"].Value.ToString(), out value);

                    if (headerType.ToString().ToLower() == "deductions")
                        decimal.TryParse(row.Cells["DeductionAmount"].Value.ToString(), out value);

                    if (headerType.ToString().ToLower() == "reimbursement")
                        decimal.TryParse(row.Cells["ReimbursmentAmount"].Value.ToString(), out value);
