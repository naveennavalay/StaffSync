...
namespace StaffSync
{
    ...
    // Final merged control: CustomMonthCalendar
    public class Calender : Control
    {
        ...
        // Context menu and printing fields
        private ContextMenuStrip _contextMenu;
        private ToolStripMenuItem _printMenuItem;
        private PrintDocument _printDocument;
        private PrintPreviewDialog _printPreviewDialog;
        private Bitmap _printBitmapCache = null; // cache the last rendered bitmap for printing
        ...
        public Calender()
        {
            ...
            InitializeContextMenuAndPrinting();
        }

        private void InitializeContextMenuAndPrinting()
        {
            // Context menu
            _contextMenu = new ContextMenuStrip();
            _printMenuItem = new ToolStripMenuItem("Print Calendar...");
            _printMenuItem.Click += PrintMenuItem_Click;
            _contextMenu.Items.Add(_printMenuItem);

            this.ContextMenuStrip = _contextMenu;

            // PrintDocument and Preview
            ...
            // Improve PrintPreviewDialog rendering on some environments
            typeof(PrintPreviewDialog).GetField("useAntiAlias", System.Reflection.BindingFlags.Instance | System.Reflection.BindingFlags.NonPublic)?
                .SetValue(_printPreviewDialog, true);
        }
        ...
        private void PrintMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                // Create a bitmap snapshot of the current control (what you see is what prints)
                GeneratePrintBitmap();
        ...
        private void GeneratePrintBitmap()
        {
            // Create a bitmap of the control's rendering.
            // Using DrawToBitmap should capture the custom painting.
            // If your environment doesn't capture everything, an alternative is to refactor OnPaint into a draw method.
            int w = Math.Max(1, this.Width);
            int h = Math.Max(1, this.Height);
            var bmp = new Bitmap(w, h);
            this.DrawToBitmap(bmp, new Rectangle(0, 0, w, h));
            _printBitmapCache = bmp;
        }

        private void PrintDocument_PrintPage(object sender, PrintPageEventArgs e)
        {
            if (_printBitmapCache == null)
            {
                GeneratePrintBitmap();
            }

            Bitmap bmpToPrint = _printBitmapCache;

            // Determine scaling to fit inside margin bounds while preserving aspect ratio
            Rectangle m = e.MarginBounds;
            float ratioX = (float)m.Width / bmpToPrint.Width;
            float ratioY = (float)m.Height / bmpToPrint.Height;
            float ratio = Math.Min(ratioX, ratioY);

            int drawWidth = (int)(bmpToPrint.Width * ratio);
            int drawHeight = (int)(bmpToPrint.Height * ratio);
            int drawX = m.Left + (m.Width - drawWidth) / 2;
            int drawY = m.Top + (m.Height - drawHeight) / 2;

            // Optional: draw a small title/header above the image (month/year centered)
            string headerText = DisplayMonth.ToString(DateFormat);
            using (Font headerFont = new Font("Segoe UI", 12f, FontStyle.Bold))
            using (Brush headerBrush = new SolidBrush(HeaderForeColor))
            {
                SizeF headerSize = e.Graphics.MeasureString(headerText, headerFont);
                float headerX = e.MarginBounds.Left + (e.MarginBounds.Width - headerSize.Width) / 2;
                float headerY = e.MarginBounds.Top - headerSize.Height - 6;
                // Only draw header if there's space above margin top (otherwise skip)
                if (headerY > e.PageBounds.Top)
                    e.Graphics.DrawString(headerText, headerFont, headerBrush, headerX, headerY);
            }

            // Draw the bitmap scaled
            e.Graphics.DrawImage(bmpToPrint, new Rectangle(drawX, drawY, drawWidth, drawHeight), new Rectangle(0, 0, bmpToPrint.Width, bmpToPrint.Height), GraphicsUnit.Pixel);
        ...
        {
            base.OnPaint(e);
            DrawCalendar(e.Graphics, new Rectangle(0, 0, this.ClientSize.Width, this.ClientSize.Height));
        }

        // Centralized draw routine so we can draw both to screen and capture to bitmap
        private void DrawCalendar(Graphics g, Rectangle bounds)
        {
            ...
            // Scale layout relative to bounds (we follow your original layout logic but within the bounds)
            int cellWidth = Math.Max(1, bounds.Width / 7);
            int headerHeight = ShowHeader ? 30 : 0;
            int dayHeaderHeight = 20;
            int cellHeight = Math.Max(1, (bounds.Height - headerHeight - dayHeaderHeight) / 6);

            Rectangle headerRect = new Rectangle(bounds.Left, bounds.Top, 7 * cellWidth, headerHeight);
            if (ShowHeader)
            {
            ...
            for (int i = 0; i < 7; i++)
            {
                Rectangle rect = new Rectangle(bounds.Left + i * cellWidth, bounds.Top + headerHeight, cellWidth, dayHeaderHeight);
            ...
            for (int row = 0; row < 6; row++)
            {
                for (int col = 0; col < 7; col++)
                {
                    ...
                    Rectangle cellRect = new Rectangle(bounds.Left + col * cellWidth, bounds.Top + headerHeight + dayHeaderHeight + row * cellHeight, cellWidth, cellHeight);
            ...
            if (ShowGridLines)
            {
                ...
                {
                    for (int i = 0; i <= 7; i++)
                        g.DrawLine(pen, bounds.Left + i * cellWidth, bounds.Top + headerHeight + dayHeaderHeight, bounds.Left + i * cellWidth, bounds.Top + headerHeight + dayHeaderHeight + 6 * cellHeight);
                    for (int i = 0; i <= 6; i++)
                        g.DrawLine(pen, bounds.Left, bounds.Top + headerHeight + dayHeaderHeight + i * cellHeight, bounds.Left + 7 * cellWidth, bounds.Top + headerHeight + dayHeaderHeight + i * cellHeight);
                }
            }
            ...
            if (ShowLegend)
            {
                ...
                if (grouped.Any())
                {
                    ...
                    {
                        case LegendAlignment.TopLeft: x = bounds.Left + 5; y = bounds.Top + 5; break;
                        case LegendAlignment.TopCenter: x = bounds.Left + (bounds.Width - legendWidth) / 2; y = bounds.Top + 5; break;
                        case LegendAlignment.TopRight: x = bounds.Right - legendWidth - 5; y = bounds.Top + 5; break;
                        case LegendAlignment.BottomLeft: x = bounds.Left + 5; y = bounds.Bottom - legendHeight - 5; break;
                        case LegendAlignment.BottomCenter: x = bounds.Left + (bounds.Width - legendWidth) / 2; y = bounds.Bottom - legendHeight - 5; break;
                        case LegendAlignment.BottomRight: x = bounds.Right - legendWidth - 5; y = bounds.Bottom - legendHeight - 5; break;
                    }
        ...
        // Refactored small helper to keep drawing tidy
        private void DayNumberAndCustomDraw(Graphics g, DateTime currentDate, Rectangle cellRect, ref int day)
        {
            // Background fill from DayStyles
            CalendarDayStyle style = null;
            DayStyles.TryGetValue(currentDate, out style);
            ...
            // Custom text
            if (!string.IsNullOrWhiteSpace(style?.CustomText))
            {
