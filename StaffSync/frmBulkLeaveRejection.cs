...
namespace StaffSync
{
    public partial class frmBulkLeaveRejection : Form
    {
        ...
        public frmBulkLeaveRejection()
        {
            InitializeComponent();
        }

        public frmBulkLeaveRejection(UserRolesAndResponsibilitiesInfo objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo)
        {
            InitializeComponent();
            objTempCurrentlyLoggedInUserInfo = objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo;
        }

        public frmBulkLeaveRejection(UserRolesAndResponsibilitiesInfo objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo, ClientFinYearInfo objSelectedClientFinYearInfo)
        {
            InitializeComponent();
            objTempCurrentlyLoggedInUserInfo = objCurrentlyLoggedInUserRolesAndResponsibilitiesInfo;
        ...
        private void frmBulkLeaveRejection_Load(object sender, EventArgs e)
        {
        ...
        private void btnSaveDetails_Click(object sender, EventArgs e)
        {
            if (this.dtgBulkLeaveApproval.Rows.Count > 0)
            {
                if (MessageBox.Show("You are about to execute Bulk Leave Rejection. \nAre you sure to continue?", "Warning", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.No)
                {
                    return;
                }
            }
            ...
            foreach (DataGridViewRow indRow in this.dtgBulkLeaveApproval.Rows)
            {
                ...
                {
                    employeeLeaveTRID = objLeaveTRList.ApproveLeave(Convert.ToInt16(indRow.Cells["LeaveTRID"].Value.ToString()), Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), "Request Rejected via Bulk Rejection", clsCurrentUser.UserID);
                    if (Convert.ToBoolean(indRow.Cells["Canceled"].Value.ToString()) == false)
                    {
                        EmployeeAttendanceInfo objEmployeeAttendanceInfo = objAttendanceMas.GetEmployeeSpecificDailyAttendanceInfo(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()));
                        if (objEmployeeAttendanceInfo.AttStatus == null)
                        {
                            objAttendanceInfo.InsertDailyAttendance(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()), "Present", 0);
                        }
                        else if (objEmployeeAttendanceInfo.AttStatus == "Present")
                        {
                            objAttendanceInfo.UpdateDailyAttendance(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()), "Present", 0);
                        }
                        else
                        {
                            strAttendanceStatus = objEmployeeAttendanceInfo.AttStatus + ", " + strAttendanceStatus;
                            //objAttendanceInfo.InsertDailyAttendance(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()), "Leave : " + lstLeaveTRList.SelectedItems[0].SubItems[6].Text.ToString(), Convert.ToInt16(indRow.Cells["LeaveTRID"].Value.ToString()));
                            RowCounter = objAttendanceInfo.UpdateDailyAttendance(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()), strAttendanceStatus, Convert.ToInt16(indRow.Cells["LeaveTRID"].Value.ToString()));
                        }
                    }

                    List<MonthlyAttendanceInfo> objMonthlyAttInfoList = objEmpMnthlyAttdInfo.getEmployeeMonthlyAttendanceInfo(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime("01" + Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()).Date.ToString("-MMM-yyyy")));
                    if (objMonthlyAttInfoList.Count == 0)
                        MonthlyAttendanceSlNumber = objEmpMnthlyAttdInfo.InsertMonthlyAttendanceInfo(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime("01" + Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()).Date.ToString("-MMM-yyyy")));

                    MonthlyAttendanceSlNumber = objMonthlyAttInfoList[0].SlNo;
                    MonthlyAttendanceSlNumber = objEmpMnthlyAttdInfo.UpdateMonthlyAttendanceInfo(MonthlyAttendanceSlNumber, Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), dtSelectedMonth, "Day" + Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()).Date.Day, strAttendanceStatus);

                    List<LeaveEntitlementInfo> objLeaveEntitlementInfo = objEmpLeaveEntitlementInfo.getEmployeeLeaveEntitilementList(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToInt16(indRow.Cells["LeaveEntmtID"].Value.ToString()));
                    if(objLeaveEntitlementInfo.Count > 0)
                        objEmpLeaveEntitlementInfo.UpadateSpecificLeaveBalanceOnly(Convert.ToInt16(indRow.Cells["LeaveEntmtID"].Value.ToString()), Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDecimal(Convert.ToDecimal(objLeaveEntitlementInfo[0].BalanceLeaves) + Convert.ToDecimal(indRow.Cells["LeaveDuration"].Value.ToString())));
                }

                //if (Convert.ToBoolean(indRow.Cells["Select"].Value.ToString()) == true)
                //{
                //    employeeLeaveTRID = objLeaveTRList.RejectLeave(Convert.ToInt16(indRow.Cells["LeaveTRID"].Value.ToString()), Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), "Request Rejected via Bulk Rejection", clsCurrentUser.UserID);
                ...
                //        //objAttendanceInfo.InsertDailyAttendance(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()), "Leave : " + indRow.Cells["LeaveMode"].Value.ToString(), Convert.ToInt16(indRow.Cells["LeaveTRID"].Value.ToString()));
                //    }
                ...
                //    txtAvailableLeave = Convert.ToDecimal(objLeaveTRList.getSpecificLeaveTypeBalance(Convert.ToInt16(lblLeaveMasID), Convert.ToInt16(indRow.Cells["LeaveTypeID"].Value.ToString())).ToString());
                //    txtBalanceLeave = Convert.ToDecimal(txtAvailableLeave) + Convert.ToDecimal(indRow.Cells["LeaveDuration"].Value.ToString());
                //    int empLeaveEntitlementID = objEmpLeaveEntitlementInfo.UpadateLeaveEntitlementInfo(Convert.ToInt16(indRow.Cells["LeaveEntmtID"].Value.ToString()), Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToInt16(lblLeaveMasID.ToString()), Convert.ToInt16(indRow.Cells["LeaveTypeID"].Value.ToString()), Convert.ToDecimal(txtBalanceLeave), Convert.ToDecimal(txtBalanceLeave), 1);

                //    //List<MonthlyAttendanceInfo> objMonthlyAttInfoList = objEmpMnthlyAttdInfo.getEmployeeMonthlyAttendanceInfo(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime("01" + Convert.ToDateTime(txtLeaveApprovalDate.Text.ToString()).Date.ToString("-MMM-yyyy")));
                //    //if (objMonthlyAttInfoList.Count == 0)
                //    //    MonthlyAttendanceSlNumber = objEmpMnthlyAttdInfo.InsertMonthlyAttendanceInfo(Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime("01" + Convert.ToDateTime(txtLeaveApprovalDate.Text.ToString()).Date.ToString("-MMM-yyyy")));

                //    //MonthlyAttendanceSlNumber = objMonthlyAttInfoList[0].SlNo;
                //    //MonthlyAttendanceSlNumber = objEmpMnthlyAttdInfo.UpdateMonthlyAttendanceInfo(MonthlyAttendanceSlNumber, Convert.ToInt16(indRow.Cells["EmpID"].Value.ToString()), Convert.ToDateTime(txtLeaveApprovalDate.Text.ToString()), "Day" + Convert.ToDateTime(indRow.Cells["ActualLeaveDateFrom"].Value.ToString()).Date.Day, "Leave : " + indRow.Cells["LeaveMode"].Value.ToString());
                //}
        ...
        private void FormatTheGrid()
        {
            ...
            dtgBulkLeaveApproval.Columns["LeaveEntmtID"].ReadOnly = true;
            dtgBulkLeaveApproval.Columns["LeaveEntmtID"].Width = 100;
            dtgBulkLeaveApproval.Columns["LeaveEntmtID"].Visible = false;
        ...
        private void frmBulkLeaveRejection_KeyDown(object sender, KeyEventArgs e)
        {
        ...
        private void frmBulkLeaveRejection_Activated(object sender, EventArgs e)
        {
            dtgBulkLeaveApproval.StateCommon.HeaderColumn.Content.Font = new System.Drawing.Font("Segoe UI", 8F, FontStyle.Bold);
        }

        private void dtgBulkLeaveApproval_Paint(object sender, PaintEventArgs e)
        {
